...
/// EAVT Transactor binding. Exposes only EAVT-level operations (save, retract,
/// declare_attr, resolver queries, allocation). For raw KV access use
/// `spier-kvstore-py` instead.
#[pyclass(name = "Engine")]
pub struct PyEngine {
...
impl PyEngine {
    ...
    fn resolve_as_of(&self, py: Python<'_>, as_of_us: u64) -> PyResult<Option<u64>> {
        py.allow_threads(|| self.inner.resolve_as_of(as_of_us))
            .map_err(to_string_err)
    }
}
