...
fn decode_value_fixed_vt(
    a_id: u32,
    bits: u64,
    vt_for: &mut impl FnMut(u32) -> Option<u32>,
) -> Value {
    match vt_for(a_id) {
...
pub fn unpack_key_with_vt(
    cf: &str,
    key: &[u8],
    mut vt_for: impl FnMut(u32) -> Option<u32>,
) -> RawDatom {
...
mod tests {
    ...
    fn test_int64_ordering_preserved() {
        let pairs: Vec<(i64, i64)> =
            vec![(0, 1), (-1, 0), (-1, 1), (i64::MIN, i64::MAX), (-100, 100)];
        for (a, b) in pairs {
            assert!(
    ...
    fn test_build_entries_variable() {
        ...
        for (cf, key, _) in &ie.entries {
            assert!(
                key.contains(&0),
                "variable entry for {} should have null separator",
                cf
            );
        }
    }
    ...
    fn test_build_entries_unordered_bytes() {
        ...
        for (cf, key, _) in &ie.entries {
            assert!(
                key.len() > 28,
                "unordered bytes entry for {} should be variable-length",
                cf
            );
        }
    }
    ...
    fn test_build_prefix_eavt_entity_attr() {
        let result = build_prefix(
            "EAVT",
            &[BoundValue::Int(42), BoundValue::Attr(5)],
            EncodeMode::Fixed,
        );
