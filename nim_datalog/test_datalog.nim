...
suite "translate: simple SELECT":
  test "eid param binding":
    ...
    check ir.findVars[0].varName == "?v_d1_company_s_name"
    ...
    check ir.patterns[0].v.varName == "?v_d1_company_s_name"
    check not ir.star
  ...
  test "select without where":
    ...
    check ir.patterns[0].e.varName == "?e_d1"
    check ir.patterns[0].a.kind == dsConst
    check ir.patterns[0].a.constVal.kind == bvAttr
    check ir.patterns[0].a.constVal.attrName == "company/name"
  ...
  test "star expansion":
    ...
    check ir.findVars[0].kind == fvVar
    check ir.findVars[0].varName == "?e_d1"
    check ir.findVars[1].kind == fvVar
    check ir.findVars[1].varName == "?a_d1"
    check ir.findVars[2].kind == fvVar
    check ir.findVars[2].varName == "?vv_d1"
...
suite "translate: joins":
  test "entity join via field ref":
    ...
    check ir.patterns[0].v.kind == dsVar and ir.patterns[0].v.varName == "?e_d2"
    check ir.patterns[1].e.kind == dsVar and ir.patterns[1].e.varName == "?e_d2"
  ...
  test "multi-projection":
    ...
    check ir.findVars[1].kind == fvVar
    check ir.findVars[1].varName == "?v_d1_company_s_name"
    check ir.findVars[2].kind == fvVar
...
suite "translate: OR":
  test "or condition":
    ...
    let ir = buildDatalogIr(stmt)
    check ir.rangeBounds.hasKey("?e_d1")
    check ir.rangeBounds["?e_d1"].len == 2
