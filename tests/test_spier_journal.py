...
def journal_instance(tmp_path):
    handle = spier_transactor_py.Journal({"path": str(tmp_path)})
    yield handle
    handle.close()
...
class TestJournalSchemaReflection:
    def test_typed_methods_exist(self):
        cls = spier_transactor_py.Journal
...
class TestJournalAppendRead:
    ...
    def test_append_multiple_entries(self, journal_instance):
        for i in range(5):
    ...
    def test_append_empty_key_value(self, journal_instance):
        journal_instance.journal_append(**{"key": b"", "value": b""})
        entries = _unpack_kv(bytes(journal_instance.journal_read()))
        assert entries == [(b"", b"")]

    def test_append_binary_data(self, journal_instance):
...
class TestJournalTruncate:
    ...
    def test_truncate_when_empty_is_ok(self, journal_instance):
        journal_instance.journal_truncate()
...
class TestJournalFileOnDisk:
    ...
    def test_journal_binary_format(self, journal_instance, tmp_path):
    ...
    def test_journal_removed_after_truncate(self, journal_instance, tmp_path):
...
class TestJournalPersistence:
    def test_entries_survive_instance_recreate(self, tmp_path):
        h1 = spier_transactor_py.Journal({"path": str(tmp_path)})
        h1.journal_append(**{"key": b"persist", "value": b"data"})
        h1.close()

        h2 = spier_transactor_py.Journal({"path": str(tmp_path)})
