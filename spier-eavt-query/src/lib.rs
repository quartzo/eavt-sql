...
pub use spier_page_store_nim::transactor::ValueType;
...
impl<'a> CompileStats for TxStats<'a> {
    ...
    fn is_ref_attr(&self, name: &str) -> bool {
        use spier_page_store_nim::transactor::resolver_consts::DB_TYPE_REF;
        if let Some(aid) = TransactorEngine::lookup_attr(self.tx, name).ok().flatten() {
            TransactorEngine::value_type_for(self.tx, aid)
                .ok()
                .flatten()
                .map(|vt| spier_page_store_nim::transactor::value_type_to_eid(vt) == DB_TYPE_REF)
                .unwrap_or(false)
        } else {
            false
        }
...
impl QueryEngine for QueryState {
    ...
    fn scan_datoms(&self, as_of_us: u64) -> Result<Vec<u8>, String> {
        ...
        let mut pos = 0;
        let mut all: Vec<spier_page_store_nim::transactor::keys::RawDatom> = Vec::new();
        while pos + 4 <= eavt_keys.len() {
            ...
            let raw = spier_page_store_nim::transactor::keys::unpack_key_with_vt("eavt", key, |aid| {
                engine
                    .tx()
                    .value_type_for(aid)
                    .ok()
        ...
        use std::collections::HashMap;
        let mut groups: HashMap<(u64, u32, Value), spier_page_store_nim::transactor::keys::RawDatom> =
            HashMap::new();
        for raw in all {
        ...
        let mut active: Vec<&spier_page_store_nim::transactor::keys::RawDatom> =
